...
export default function FurnitureCatalog() {
  ...
  const items = FURNITURE.filter((f) => {
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="grid flex-1 grid-cols-2 content-start gap-2.5 overflow-y-auto p-3">
        {items.map((f) => {
          const Icon = ICONS[CATEGORY_META[f.category].icon] ?? Square;
          return (
            <button
              key={f.id}
            ...
            >
              <div className="grid h-12 w-12 place-items-center rounded-lg bg-brand-500/10 text-brand-300 ring-1 ring-inset ring-white/5 transition-colors group-hover:bg-brand-500/20">
              ...
              <span className="absolute right-1.5 top-1.5 grid h-6 w-6 place-items-center rounded-md bg-brand-500 text-white opacity-0 shadow transition-opacity group-hover:opacity-100">
